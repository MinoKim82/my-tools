...
help:
	...
	@echo "  make link tool=<name> [as=<alias>]  - Symlink CLI entry point to $(BIN_DIR)/<name> (or alias)"
	@echo "  make unlink tool=<name> [as=<alias>]- Remove CLI symlink from $(BIN_DIR)/<name>"
...
link:
	@if [ -z "$(tool)" ]; then \
		echo "Error: Please specify tool=<name>, e.g.: make link tool=my-script [as=alias]"; \
		exit 1; \
	fi
	@mkdir -p $(BIN_DIR)
	@tool_dir="tools/$(tool)"; \
	...
	chmod +x "$$entry"; \
	ln -sf "$$(pwd)/$$entry" "$(BIN_DIR)/$$bin_name"; \
	echo "Successfully linked CLI: $$(pwd)/$$entry -> $(BIN_DIR)/$$bin_name"

unlink:
	@if [ -z "$(tool)" ]; then \
		echo "Error: Please specify tool=<name>, e.g.: make unlink tool=my-script [as=alias]"; \
		exit 1; \
	fi
	@bin_name="$${as:-$(tool)}"; \
	if [ -L "$(BIN_DIR)/$$bin_name" ]; then \
		rm "$(BIN_DIR)/$$bin_name"; \
		echo "Removed CLI symlink $(BIN_DIR)/$$bin_name"; \
	else \
		echo "No CLI symlink found at $(BIN_DIR)/$$bin_name"; \
	fi
